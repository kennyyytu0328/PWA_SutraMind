...
- `<BuddhaBackdrop />` — /chat only. Fixed photo (`public/buddha_background.jpg`) as a CSS `background-image` at `z-index: 0` (same layer as `.zen-ambient`), **portalled to `<body>`** because `<main>`'s mount-fade transform would otherwise trap the fixed layer inside the column. `.buddha-backdrop::after` vignettes every edge into `zen-bg`. No motion.
- `<AmbientAudio />` — sound toggle in `AppHeader`; loops `public/audio/般若波羅密多心經.mp3` (Heart Sutra chant) with a 1.2s volume fade, preference in `localStorage`. Path goes through `encodeURI` because of the CJK filename.
...
- ✅ Buddha photo backdrop on `/chat` (`BuddhaBackdrop`) replaces the inline-SVG moonlit scene; ambient audio now loops the Heart Sutra chant (2026-09-21)
- ✅ Moonlit backdrop on `/chat` (`MoonlitBackdrop`): fixed inline-SVG scene, gold halo, no image asset (2026-09-04, superseded 2026-09-21)
